...
export function downloadStart(
  ...
  return invoke("download_start", { gid, token, title, label, artist, language, group, total, url });
}

export type RenameStatus = "renamed" | "skipped" | "failed";
...
/** Default tags stripped when renaming gallery folders (mirrors the backend). */
export const DEFAULT_RENAME_TERMS = [
  "chinese", "english", "japanese", "korean", "french", "german", "spanish",
  "russian", "italian", "portuguese", "dutch", "taiwanese", "polish", "thai", "vietnamese",
  "chn", "eng", "jpn", "zh", "cn", "jp", "kr",
  "中文", "简体", "簡體", "繁体", "繁體", "简中", "繁中", "英语", "日语", "韩语", "中译", "中英", "日中",
  "translat", "scanlat", "汉化", "翻译", "翻訳", "简翻",
  "ai generated", "ai", "aigenerated", "autogenerated", "midjourney", "stable diffusion", "novelai", "kobold",
  "patreon", "patron", "pixiv", "pivix", "fanbox", "ko-fi", "kofi", "unifans", "fantia", "subscribe",
  "digital", "dl", "dl版", "full color", "fullcolour", "fullcolor", "webtoon", "crop", "no text", "textless",
];

/** Recursively scans the download root and returns the rename plan. `preview`
 *  only computes before/after names; `false` performs the actual renames and
 *  removes the renamed galleries from the download queue. The scan root is the
 *  persisted `rename_scan_dir` setting (empty = configured download root). */
export function renameGalleryDirs(preview: boolean): Promise<RenameReport> {
  if (isMock()) {
    return Promise.resolve({ entries: [], renamed: 0, skipped: 0, failed: 0 });
  }
  return invoke<RenameReport>("rename_download_dirs", { preview });
}
